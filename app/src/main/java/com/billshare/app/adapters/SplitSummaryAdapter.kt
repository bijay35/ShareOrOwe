...
class SplitSummaryAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        AvatarHelper.bind(holder.binding.tvAvatar, bill.paidBy)
        ...
        val isFullySettled = DataManager.isBillFullySettled(ctx, bill)
        val currentUser = DataManager.getCurrentUser(ctx)
        val isCurrentUserSettled = currentUser != null && DataManager.isPersonSettledForBill(ctx, bill.id, currentUser.id)
        ...
        when {
            ...
            isCurrentUserSettled -> {
                ...
                holder.binding.tvStatus.setTextColor(ContextCompat.getColor(ctx, R.color.amount_positive))
                holder.binding.btnSettle.isEnabled = false
                holder.binding.btnSettle.alpha = 0.4f
            }
            else -> {
    ...
    fun billAt(position: Int): SplitBill? = bills.getOrNull(position)
}
